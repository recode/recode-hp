...
var ATSnow = function (vars) {
  ...
  this.createSnow = function () {
    var body = _doc.getElementById("snow-area");
    ...
    for (var i = 0; i < options.count; i++) {
      ...
      var snows = _snows[i];
      snows.ele = snow;
      snows.distance = _self.getRandomInt(
        options.minDistance,
        options.maxDistance
      );
  ...
  this.snowPositionReset = function (snow, y) {
    var leftPosition = _self.getRandomInt(
      options.leftMargin,
      _windowWidth - options.rightMargin
    );
  ...
  this.move = function () {
    var fall = function () {
      for (var i = 0; i < options.count; i++) {
        ...
        if (
          top + snow.size + snow.size >=
          _scrollHeight - options.bottomMargin
        ) {
          ...
          snow.ele.style.left =
            snow.x +
            Math.cos((snow.move * Math.PI) / 180) * snow.distance +
            "px";
        }
      }
      setTimeout(function () {
